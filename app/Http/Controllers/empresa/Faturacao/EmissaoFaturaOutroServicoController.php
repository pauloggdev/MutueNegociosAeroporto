...
class EmissaoFaturaOutroServicoController extends Component
{
    use LivewireAlert;
    ...
    public function emitirDocumento()
    {
        ...
        $emitirDocumento = new EmitirDocumentoAeroportoOutroServico(new DatabaseRepositoryFactory());
        $faturaId = $emitirDocumento->execute(new Request($this->fatura));
        $this->printFatura($faturaId);
        $this->resetField();
    }
