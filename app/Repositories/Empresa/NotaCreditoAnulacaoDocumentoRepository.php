...
class NotaCreditoAnulacaoDocumentoRepository
{
    ...
    public function salvarNotaCreditoAnularRecibo(array $data)
    {
        ...
        $sequenciaDocumento = DB::connection('mysql2')->table('sequencias_documentos')
            ->where('tipo_documento', EnumTipoDocumento::$NOTA_CREDITO_ANULACAO_RECIBOS)
            ->where('empresa_id', auth()->user()->empresa_id)
            ->orderBy('id', 'DESC')
            ->first();
        if ($sequenciaDocumento) {
            if ($sequenciaDocumento->sequencia > $numSequencia) {
                $numSequencia = $sequenciaDocumento->sequencia;
            }
        }
    ...
    public function salvarNotaCreditoAnularFactura(array $data)
    {
        ...
        $sequenciaDocumento = DB::connection('mysql2')->table('sequencias_documentos')
            ->where('tipo_documento', EnumTipoDocumento::$NOTA_CREDITO_ANULACAO_FATURAS)
            ->where('empresa_id', auth()->user()->empresa_id)
            ->orderBy('id', 'DESC')
            ->first();
        if ($sequenciaDocumento) {
