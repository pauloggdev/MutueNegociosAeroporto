        ...
        <div class="row">
          <form id="adimitirCandidatos" method="POST" action>
            ...
            >
              ...
              >
                <a
                  title="Imprimir lista dos produtos mais vendidos"
                  v-on:click="imprimirListarProdutosMaisVendidos"
                ...
                >
                  ...
                  <tbody>
                    <tr v-for="(produto, i) in queryprodutos" :key="i">
                      ...
                      <td style="text-align: right">
                        {{
                          (produto.preco_compra ? produto.preco_compra : 0)
                            | currency
                        }}
                      </td>
                      <td class="hidden-480" style="text-align: center">
                        {{ produto.qtVendidos | formatQt }}
                      </td>
                      <td class="hidden-480" style="text-align: center">
                        <span
                          class="label label-sm label-success"
                          v-if="produto.stocavel == 'Sim'"
                          >{{ produto.stocavel }}</span
                        >
                        <span class="label label-sm label-warning" v-else>{{
                          produto.stocavel
                        }}</span>
                      </td>
                    </tr>
...
export default {
  ...
  methods: {
    async imprimirListarProdutosMaisVendidos() {
      this.$loading(true);

      try {
        let response = await axios.get(
          `${this.router}/produtos/imprimirActualizacaoStock/${existenciaId}`,
          {
