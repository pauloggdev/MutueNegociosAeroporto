...
<div class="facturaRegibo">
    <div class="row">
        <div class="modal fade" id="modalAdicionarQuantidade" wire:ignore>
            ...
            <div id="info-total">
                <div>
                    ...
                    <div class="total-item">DESCONTO
                        <span>{{ number_format($fatura['totalDesconto'], 2, ',', '.') }}</span>
                    </div>
                    <div class="total-item"> TOTAL IVA
                        <span>{{ number_format($fatura['totalIva'], 2, ',', '.') }}</span>
                    </div>
            ...
            <table class="table table-striped table-hover">
                ...
                @foreach($fatura['items'] as $produto)
                    <tr style="display: table; width: 100%; table-layout: fixed"
                        ...
                        <td style="width: 70%"> {{ Str::upper($produto['nomeProduto']) }}
            ...
            <div class="row">
                ...
                <div class="col-md-12" style="z-index: 1">
                    <div class="row" style="margin-top: 10px">
                        <!-- PESQUISA PRODUTOS  -->
                        <div class="col-md-12">
                            <label class="control-label bold label-select2" for="proodutoId">PRODUTOS/SERVIÇOS</label>
                            <select wire:model="produtoSelecionado" data="produtoSelecionado" class="col-md-12 select2">
                                <option>SELECIONA O PRODUTO</option>
                                @foreach($produtos as $produto)
                                    <option value="{{ json_encode($produto) }}">{{ Str::upper($produto['nomeProduto']) }} - {{isset($produto['codigoProduto'])?$produto['codigoProduto']:null}}</option>
                                @endforeach
                            </select>
                        </div>
        ...
        <div class="col-md-3" style="padding-right:0px;">
            <div class="row">
                <label class="control-label bold" for="preco_compra">SELECIONAR CLIENTE
                    ...
                    <a href="#modalCartaoCliente" wire:click="showModalCartaoCliente" title="ativo pagar com saldo" data-toggle="modal"
                       style="background:green; padding: 5px;border-radius: 5px;color: white; cursor: pointer;" >PAGAR COM SALDO</a>
                            <i wire:click="showCancelarPagarComSaldoCliente" style="color: red;font-size: 40px;align-items: center;top: 3px;background: green;padding: 4px;cursor:pointer;border-radius: 2px" class="ace-icon glyphicon glyphicon-remove bigger-130"></i>
                    @else
                    <a href="#modalCartaoCliente" wire:click="showModalCartaoCliente" data-toggle="modal"  title="desativo pagar com saldo"
                       style="background:red; padding: 5px;border-radius: 5px;color: white; cursor: pointer;" >PAGAR COM SALDO</a>
                    @endif
                </label>
                <select wire:model="fatura.clienteId" data="clienteId" class="col-md-12 select2" id="clienteId" <?= $fatura['aplicadoCartaoCliente']?'disabled':''?>>
                                        ...
                                        <div class="inputFormPag">
                                            ...
                                            <select style="margin-bottom: 10px;" wire:model="fatura.formaPagamentoId"
                                                ...
                                                @foreach($formasPagamento as $formaPagamento)
                                                    <option
                                                        value="{{ $formaPagamento->id }}" {{ $formaPagamento->id == 4 || $formaPagamento->id == 5?'disabled':'' }} >{{ Str::upper($formaPagamento->descricao) }}</option>
                                                @endforeach
                                            </select>
                                        </div>
    ...
    <div class="row">
        <div class="" style="float: right;">
            ...
            <button class="btn btn-app btn-primary" style="width:480px"
