...
<body class="skin-1" id="body" style="overflow-x: hidden;">
                    ...
                    <ul class="submenu">
                        ...
                        <li class="hover">
                            <a href="/empresa/produtos/quantidade/critica">
                                <i class="menu-icon fa fa-list"></i>
                                LISTAR PRODUTOS COM QUANTIDADE CRÍTICA
                            </a>
                            <b class="arrow"></b>
                        </li>
                        <!-- <li class="hover">
    ...
    <script type="text/javascript">
        ...
        window.Laravel = {
            json_encode(['csrfToken'=>csrf_token(),'roles'=>Auth::user()->roles,'user'=>[
...
$(document).ready(function() {
    ...
    <script>
        ...
        window.addEventListener('livewire:load', function () {
            Livewire.on('preventEnter', function () {
                event.preventDefault();
            });
        });

        $(document).ready(function() {
            $(document.body).on("change", "#valorParametroId", function(e){
                const data ={
            ...
            $('.selectArmazem').select2(); // Inicializa o Select2 em todos os elementos com classe "select2
            $(document.body).on("change",".selectArmazem",function(e){
                const data ={
                    'atributo':e.target.getAttribute('data'),
                    'valor':e.target.value
                }
                livewire.emit('selectedArmazem', data)
            });
            // window.livewire.on('select2',()=>{
            //     $('.select2Armazem').select2(); // Inicializa o Select2 em todos os elementos com classe "select2"
            //
            // });
        });
    </script>
    ...
    <script>
        ...
        window.addEventListener('changeSelect2', event => {
            $(`#${event.detail.id}`).empty();
            var novasOpcoes = "";
            for (let index = 0; index < event.detail.data.length; index++) {
                novasOpcoes += '<option value="' + event.detail.data[index] + '">' + event.detail.data[index].numeracaoFactura + '</option>';
                // $(`#${event.detail.id}`).val(event.detail.data[index]).trigger('change');

            }
            $(`#${event.detail.id}`).append(novasOpcoes);
        });
        window.addEventListener('showModal', event => {
            $(`#${event.detail}`).addClass('show in');
            $("#body"). append(`<div class="modal-backdrop fade show fadeShow"></div>`);
            $("#body").append(`<div class="modal-backdrop fade in fadeIn"></div>`);
        ...
        window.addEventListener('closeLoading', event => {
            $("#loading").removeClass("loading");
        })

        window.addEventListener('reloadTableJquery', event => {
                ...
                $('.tabela1').dataTable({
                    ...
                    "language": {
                        "emptyTable": "Sem dados disponivel na tabela",
                        "info": "<span class='text-primary' style='font-size: 12pt; left: 8%;'>Mostrar de _START_ a _END_ Registos(_TOTAL_ no total)</span>",
                        "infoEmpty": "Mostrar de 0 a 0 registos",
                        "infoFiltered": "(Filtrada de _MAX_  registos)",
                        "lengthMenu": "<span class='text-primary' style='font-size:13pt; position: absolute; left: 1%;'>Mostrar _MENU_ </span>",
        ...
        window.addEventListener('refreshPagina', event => {
            setTimeout(function() {
                location.reload();
            }, 2000);

        })
        window.addEventListener('printExcel', event => {
            window.open(event.detail.data);
        })
        ...
        window.addEventListener('printLinkNoBlank', event => {
            var data = event.detail.data;
            // alert(data)
            window.open(data, '_self');
        })
        ...
        window.addEventListener('closeModal', event => {
            ...
            $('.fadeShow').removeClass("modal-backdrop fade show");
            $('.fadeIn').removeClass("modal-backdrop fade in");
        })
    ...
    <script>
        ...
        $('.select10').select2().on('change', function(e) {
            window.livewire.emit('selected', e.target.id, $(this).val())

        });

    $(document).ready(function() {
        window.initSelectStatusDrop=()=>{
            $('#selectStatuItem').select2({
                placeholder: 'Informe o status',
                allowClear: false
            });
        }
        initSelectStatusDrop();
        $('#selectStatuItem').on('change', function (e) {
            var data = $('#selectStatuItem').select2("val");
            livewire.emit('selectedStatuItem', data)
        });
         window.livewire.on('select10',()=>{
            initSelectStatusDrop();
        });
    ...
    $(document).ready(function() {
        //edit user
        window.initSelectStatusDrop2=()=>{
            $('#selectMultiplo2').select2({
                placeholder: 'Informe a função',
                allowClear: false});
        }
        initSelectStatusDrop2();
        $('#selectMultiplo2').on('change', function (e) {
            var data = $('#selectMultiplo2').select2("val");
            livewire.emit('selectedFuncaoItem', data)
        });
        window.livewire.on('select100',()=>{
            initSelectStatusDrop2();
        });
        // window.livewire.on('clearSelectMultiplo',()=>{
        //     $('#selectMultiplo2').val(null).trigger('change');
        //     // initSelectStatusDrop();
        // });
        ...
        initSelectStatusDrop();
        $('#selectMultiplo').on('change', function (e) {
            var data = $('#selectMultiplo').select2("val");
            livewire.emit('selectedFuncaoItem', data)
        });
        window.livewire.on('clearSelectMultiplo',()=>{
            $('#selectMultiplo').val(null).trigger('change');
            // initSelectStatusDrop();
        });
    });
